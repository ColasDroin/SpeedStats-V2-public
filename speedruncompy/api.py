...
def doGet(endpoint: str, params: dict = {}):
    ...
    while attempt < MAX_ATTEMPTS:
        try:
            response = get(
                url=f"{getProxyUri()}{API_URI}{endpoint}",
                headers=_header,
                params={"_r": _r},
                timeout=TIMEOUT,
            )
...
def doGetV1(endpoint: str, params: dict = {}):
    ...
    while attempt < MAX_ATTEMPTS:
        try:
            response = get(
                url=f"{getProxyUri()}{API_V1_URI}{endpoint}{buildParams(params)}",
                headers=_header,
                timeout=TIMEOUT,
            )
            return response
...
class BaseRequest:
    ...
    def perform(self, retries=MAX_ATTEMPTS, delay=TIMEOUT) -> dict:
        try:
            ...
            if (
                (self.response.status_code >= 500 and self.response.status_code <= 599)
                or self.response.status_code == 408
                or self.response.status_code == 429
            ):
                if retries > 0:
                    _log.error(
                        f"SRC returned error {self.response.status_code} {self.response.content}. Retrying with delay {delay}:"
                    )
                    for attempt in range(1, retries + 1):
                        ...
                        if not (
                            ...
                            break
                        _log.error(
                            f"Retry {attempt} returned error {self.response.status_code} {self.response.content}"
                        )
                        sleep(delay)
                    else:
                        if self.response.status_code == 408:
                            raise RequestTimeout(self)
                        elif self.response.status_code == 429:
                            raise RateLimitExceeded(self)
                        else:
                            raise ServerException(self)

            if self.response.status_code == 400:
                raise BadRequest(self)
            ...
            if self.response.status_code == 404:
                raise NotFound(self)
            if self.response.status_code == 405:
            ...
            if self.response.status_code < 200 or self.response.status_code > 299:
                _log.error(
                    f"Unknown response error returned from SRC! {self.response.status_code} {self.response.content}"
                )
